...
router.post("/product", upload.single('image'), async (req, res) => {
  try {
    ...
    if(productPrice < 0 || productStock < 0){
      return res.status(400).json({message: "Price or Stock must not be negative"})
    }

    const newImagePath = `uploads/${req.file.filename}`
    const newProduct = { productName, productDescription, productPrice, productImagePath: newImagePath, productCategory, productStock };
...
router.patch("/product/:id", async (req, res) => {
  try {
    const productId = req.params.id;
    const { productName, productDescription, productPrice, productImage, productCategory, productStock,} = req.body;
    if ( !productName && !productDescription && !productPrice && !productImage && !productCategory && !productStock ) {
      return res.status(400).json({ message: "Request is incomplete" });
    }
